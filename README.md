...
okaylib is a personal project which is intended to focus many disparate efforts
of mine to make C and C++ libraries into one mega-project. I have plan to use
it myself in some of my other projects, but any actual releases (along with
...
- [x] `<type_traits>` reimplementation
- [x] `<tuple>` reimplementation
...
- [x] A dynamic bit array and a static bit array with boolean-like iterators, to
...
- [x] Add user-defined error values to the result. Also add some kind of anyhow
      error type result.
...
- [x] A `std::vector` replacement with a better name (`ok::arraylist`?) which
...
- [x] A `std::inplace_vector` replacement: `ok::fixed_arraylist_t`
- [ ] A collection whose items can be accessed by a stable handle, instead of
      ...
      debugging.
- [x] An arraylist type which does not store its elements contiguously but rather
...
- [x] an allocator aware `unique_ptr` replacement
- [x] an allocator aware `shared_ptr` replacement
