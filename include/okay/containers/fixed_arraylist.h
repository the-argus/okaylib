...
class fixed_arraylist_t
    : protected stdc::conditional_t<
          stdc::is_trivially_destructible_v<T>,
          ...
          detail::fixed_arraylist_nontrivial_destruction_t<T, max_elems>>
{
  ...
  public:
    ...
    template <size_t num_elems_in_initializer>
        requires(num_elems_in_initializer <= max_elems)
    ...
    template <size_t other_max_elems>
        requires(other_max_elems <= max_elems)
        ...
        OKAYLIB_NOEXCEPT
    {
        ...
        if constexpr (stdc::is_trivially_copyable_v<T>) {
            ...
            for (size_t i = 0; i < other.size(); ++i) {
                new (this->data() + i) T(stdc::move(other.data()[i]));
            }
        }
    ...
    {
        this->destroy();
        new (this) fixed_arraylist_t(stdc::move(other));
        return *this;
    }
    ...
    template <typename... args_t>
        requires stdc::is_constructible_v<T, args_t...>
    [[nodiscard]] constexpr auto insert_at(const size_t idx,
        ...
        stdc::conditional_t<
            stdc::is_void_v<decltype(ok::make_into_uninitialized<T>(
                ...
                stdc::declval<T&>(), stdc::forward<args_t>(args)...))>
    {
        ...
        constexpr bool make_returns_status = !stdc::is_void_v<make_result_type>;
        ...
        if (this->size() == max_elems) [[unlikely]] {
            if constexpr (make_returns_status) {
                static_assert(detail::is_instance_c<make_result_type, status>);
        ...
        if constexpr (make_returns_status) {
            ...
            if (result.okay()) [[likely]] {
                ...
                if constexpr (stdc::is_trivially_copyable_v<T>) {
                    ::memmove(this->data() + idx, this->data() + idx + 1,
                              (this->size() - idx) * sizeof(T));
                } else {
    ...
    constexpr opt<T> remove_and_swap_last(size_t idx) OKAYLIB_NOEXCEPT
    {
        ...
        new (ok::addressof(target))
            T(stdc::move(this->data()[this->size() - 1]));
    ...
    template <typename... args_t>
        requires ok::is_infallible_constructible_c<T, args_t...>
    constexpr void resize(size_t new_size, args_t&&... args) OKAYLIB_NOEXCEPT
    {
