                               ...
                               std::remove_reference_t<input_iterable_t>>>>
{
  ...
  public:
    ...
    static constexpr size_t end(const iterable_t& i) OKAYLIB_NOEXCEPT
    {
        return size(i);
    }
...
template <typename T> struct iterable_definition_inner_meta_t
{
    ...
    using inherited_id = typename has_inherited_iterable_type<noref>::type;
    using type = std::conditional_t<
        has_inherited_iterable_type_v<std::remove_reference_t<T>>,
        iterable_definition<inherited_id>, iterable_definition<T>>;
    static_assert(!has_inherited_iterable_type_v<noref> ||
                      is_derived_from_v<noref, inherited_id>,
...
using iterable_definition_inner =
    typename iterable_definition_inner_meta_t<T>::type;
...
constexpr bool is_valid_sentinel_v =
...
struct iterable_has_begin<
    ...
    std::enable_if_t<!std::is_same_v<
        void,
        decltype(iterable_definition_inner<std::remove_reference_t<T>>::begin(
            std::declval<const std::remove_reference_t<T>&>()))>>>
    : public std::true_type
{};

template <typename T, typename = void>
struct iterable_has_end : public std::false_type
{};
template <typename T>
struct iterable_has_end<
    T, std::enable_if_t<!std::is_same_v<
           void,
           decltype(iterable_definition_inner<std::remove_reference_t<T>>::end(
               std::declval<const std::remove_reference_t<T>&>()))>>>
    : public std::true_type
{};
...
struct iterable_has_size<
    T, std::enable_if_t<std::is_same_v<
           size_t,
           decltype(iterable_definition_inner<std::remove_reference_t<T>>::size(
               std::declval<const std::remove_reference_t<T>&>()))>>>
    : public std::true_type
{};
...
struct iterable_marked_infinite<
    T, std::enable_if_t<iterable_definition_inner<remove_cvref_t<T>>::infinite>>
    : public std::true_type
{};
...
struct iterable_marked_finite<
    T,
    std::enable_if_t<!iterable_definition_inner<remove_cvref_t<T>>::infinite>>
    : public std::true_type
{};
...
{};
template <typename T>
struct iterable_has_data<
    T, std::enable_if_t<std::is_pointer_v<
           decltype(iterable_definition_inner<remove_cvref_t<T>>::data(
...
template <typename T>
constexpr bool iterable_has_end_v = iterable_has_end<T>::value;
template <typename T>
...
using cursor_type_unchecked_for =
    decltype(iterable_definition_inner<T>::begin(std::declval<const T&>()));
template <typename T>
using sentinel_type_unchecked_for =
    decltype(iterable_definition_inner<T>::end(std::declval<const T&>()));
...
template <typename T>
class iterable_for
    : public detail::iterable_definition_inner<std::remove_reference_t<T>>
{
    using noref = std::remove_reference_t<T>;
    ...
    static constexpr bool has_end = detail::iterable_has_end_v<noref>;
    static_assert(
        has_end,
    ...
    static constexpr bool valid_sentinel =
